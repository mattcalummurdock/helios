#!/usr/bin/env python3
"""Download WHU-CD building change detection dataset (256px tiles)."""
...
# Pre-processed 256x256 tiles used by many CD papers (RFANet, TinyCD-SIA, etc.)
WHU_URLS = [
    "https://www.dropbox.com/s/r76a00jcxp5d3hl/WHU-CD-256.zip?dl=1",
]
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=DATASETS_DIR / "whu_cd")
    args = parser.parse_args()

    args.output.mkdir(parents=True, exist_ok=True)
    zip_path = args.output / "WHU-CD-256.zip"
    ...
    if not zip_path.exists() or zip_path.stat().st_size < 1_000_000:
        ...
        if last_err:
            print(
                "WHU-CD auto-download failed. Manual: "
    ...
    marker.touch()
    print(f"WHU-CD extracted to {args.output}")
